...
def extract_and_clean_data(engine):
...
def run_statistical_tests(df):
    ...
    results["T-Test"] = {"p": p_t, "d": (amman.mean() - irbid.mean()) / pooled_std}
...
def create_visualizations(df):
    ...
    sns.barplot(x=city_rev.values, y=city_rev.index, hue=city_rev.index, legend=False)
    ...
    sns.barplot(x=cat_aov_mean.index, y=cat_aov_mean.values, hue=cat_aov_mean.index, legend=False)
...
def main():
    try:
        engine = connect_db()
        df = extract_and_clean_data(engine)
        stats_out = run_statistical_tests(df)
        
        print(" SUMMARY STATISTICS ")
        print(f"Total Cleaned Revenue: {df['total_price'].sum():.2f} JOD")
